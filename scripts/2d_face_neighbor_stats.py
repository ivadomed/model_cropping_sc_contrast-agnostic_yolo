#!/usr/bin/env python3
"""
Per-face SI-neighbor gap statistics from GT labels in processed/.

For each of the 6 faces (R, L, A, P, I, S) and k=1..max_k:
  - Collect the face value for every detected slice
  - Order by SI position (Superior → Inferior = ascending slice_idx)
  - Compute max |f_{i+hop} - f_i| for hop=1..k over all i (cumulative)

This measures how much the bounding box face position varies between
detections that are k SI-hops apart.

Face definitions (axial LAS slices):
  R : (cx + w/2) * W_mm
  L : (cx - w/2) * W_mm
  A : (cy - h/2) * H_mm   (cy=0 = Anterior)
  P : (cy + h/2) * H_mm
  S : slice_idx * si_res   (Superior = smallest slice_idx)
  I : slice_idx * si_res   (Inferior = largest slice_idx)

Outputs (in processed_stats/<variant>/face_neighbor_gap/):
  face_neighbor_gap_k{k}.csv         — one row per patient
  violin_face_neighbor_gap_k{k}.png  — 6-subplot violin, one violin per dataset

Usage:
    python scripts/2d_face_neighbor_stats.py --processed processed/10mm_SI_1mm_axial --max-k 3
    python scripts/2d_face_neighbor_stats.py --processed processed/10mm_SI_1mm_axial --max-k 5 --exclude-csv /home/quentinr/bad_gt.csv
"""
...
FACE_LABELS = {
    ...
    "A_gap_mm": "Anterior",
    "P_gap_mm": "Posterior",
    "I_gap_mm": "Inferior (SI)",
    "S_gap_mm": "Superior (SI)",
}
...
def si_neighbor_gap(values_si_ordered: list[float], k: int) -> float:
    """Max |f_{i+hop} - f_i| for hop=1..k, values in SI order (Superior→Inferior)."""
    if len(values_si_ordered) < 2:
        return 0.0
    v = values_si_ordered
    return max(
        abs(v[i + hop] - v[i])
        for hop in range(1, min(k, len(v) - 1) + 1)
        for i in range(len(v) - hop)
    )


def read_patient_faces_si_ordered(txt_dir: Path, W_mm: float, H_mm: float,
                                   si_res: float) -> dict[str, list[float]] | None:
    """Collect per-face values ordered by SI position. Returns None if < 2 detections."""
    detections = []
    for txt_file in sorted(txt_dir.glob("slice_*.txt")):
        ...
        idx = int(m.group(1))
        detections.append((idx, cx, cy, w, h))

    if len(detections) < 2:
        return None

    detections.sort(key=lambda d: d[0])  # Superior → Inferior

    return {
        "R":  [(cx + w / 2) * W_mm for _, cx, cy, w, h in detections],
        "L":  [(cx - w / 2) * W_mm for _, cx, cy, w, h in detections],
        "A":  [(cy - h / 2) * H_mm for _, cx, cy, w, h in detections],
        "P":  [(cy + h / 2) * H_mm for _, cx, cy, w, h in detections],
        "SI": [idx * si_res          for idx, cx, cy, w, h in detections],
    }


def patient_gaps(faces: dict[str, list[float]], k: int) -> dict:
    return {
        "R_gap_mm": si_neighbor_gap(faces["R"],  k),
        "L_gap_mm": si_neighbor_gap(faces["L"],  k),
        "A_gap_mm": si_neighbor_gap(faces["A"],  k),
        "P_gap_mm": si_neighbor_gap(faces["P"],  k),
        "I_gap_mm": si_neighbor_gap(faces["SI"], k),
        "S_gap_mm": si_neighbor_gap(faces["SI"], k),
    }
...
def plot_violin(df: pd.DataFrame, out_path: Path, k: int, dpi: int) -> None:
    ...
    axes      = axes.flatten()
    fig.suptitle(f"Per-face SI-neighbor gap (hops 1..{k})", fontsize=13, fontweight="bold")
...
def main():
    parser = argparse.ArgumentParser(description="Per-face SI-neighbor gap statistics from GT labels.")
    ...
    parser.add_argument("--max-k",       type=int, required=True, dest="max_k",
                        help="compute for k=1..max_k SI hops")
    parser.add_argument("--dpi",         type=int, default=150)
    args = parser.parse_args()

    processed = Path(args.processed)
    out_dir   = Path("processed_stats") / processed.name / "face_neighbor_gap"
    ...
    for dataset in tqdm(sorted(p.name for p in processed.iterdir() if p.is_dir()), desc="datasets"):
        for patient_dir in sorted((processed / dataset).iterdir()):
            ...
            faces = read_patient_faces_si_ordered(txt_dir, W_mm, H_mm, float(meta["si_res_mm"]))
            if faces is None:
                continue
    ...
    for k in range(1, args.max_k + 1):
        ...
        csv_path = out_dir / f"face_neighbor_gap_k{k}.csv"
        df.to_csv(csv_path, index=False)
        print(f"k={k}: saved {len(df)} patients -> {csv_path}")
        plot_violin(df, out_dir / f"violin_face_neighbor_gap_k{k}.png", k=k, dpi=args.dpi)
