#!/usr/bin/env python3
"""
Find worst-performing volumes per dataset, ranked independently per metric:
  iou_3d_mm    : lowest 3D IoU first
  gap_mm_*     : highest positive gap first (pred misses GT on that face)
  gap_mm_*_neg : most negative gap first (pred over-extends on that face)
...
METRICS = {
    "iou_3d_mm":    True,    # ascending=True  → lowest (worst) first
    "gap_mm_R":     False,   # ascending=False → highest positive gap (pred misses GT) first
    "gap_mm_L":     False,
    "gap_mm_P":     False,
    "gap_mm_A":     False,
    "gap_mm_I":     False,
...
METRIC_COLUMN = {m: m for m in METRICS}
...
def make_overview(pred_root: Path, dataset: str, stem: str, out_path: Path,
    ...
    draw.text((8, 4),  f"{ordinal(rank)} worst — {dataset}   {metric_name} = {val_str}   (conf≥{conf_thresh})", fill=(255, 255, 255), font=font_large)
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Top-K worst volumes per dataset for iou_3d_mm and gap_mm_*",
