...
Creates flat per-slice symlinks for images and label files into datasets/:
  datasets/images/{train,val,test}/<dataset>_<subject>_<acq>_slice_NNN[_rN].png  (symlinks)
  datasets/labels/{train,val,test}/<dataset>_<subject>_<acq>_slice_NNN[_rN].txt  (symlinks or filtered copies)
  datasets/dataset.yaml
  --filter-classes : write filtered label copies (not symlinks) keeping only listed class IDs.
                     Use when processed/ contains multi-class txts but training needs a subset.
...
Oversampling (train split only, requires --regions-csv):
  ...
  --datasets : restrict to these dataset names (default: all from splits-dir)
...
Usage:
    ...
    python scripts/build_dataset.py --processed processed/10mm_SI_1mm_axial_3ch_canal \\
        --out datasets/10mm_SI_1mm_axial_3ch_canal \\
        --nc 2 --class-names sc canal \\
        --datasets data-multi-subject spider-challenge-2023 whole-spine
    python scripts/build_dataset.py --processed processed/10mm_SI_1mm_axial_3ch_canal \\
        --out datasets/10mm_SI_1mm_axial_3ch_1cls_v2 \\
        --nc 1 --class-names sc \\
        --filter-classes 0 \\
        --regions-csv data/dataset_regions.csv --balance-regions --dataset-factors sci-zurich:5
"""
...
def make_symlinks(slices: list[dict], images_out: Path, labels_out: Path,
                  n_copies_fn, filter_classes: Optional[set] = None) -> int:
    """Create symlinks (images) and label files for each slice. Returns total links.

    filter_classes: when set, write filtered txt files (not symlinks) keeping only listed class IDs.
    """
    n = 0
    for s in slices:
        ...
        for i in range(n_copies):
            ...
            stem     = f"{s['prefix']}_{s['png'].stem}{suffix}"
            img_link = images_out / f"{stem}.png"
            lbl_out  = labels_out / f"{stem}.txt"
            ...
            if filter_classes is None:
                ...
                lines = [
                    line for line in s["txt"].read_text().splitlines()
                    if line.split() and int(line.split()[0]) in filter_classes
                ]
                lbl_out.write_text("\n".join(lines) + ("\n" if lines else ""))
            n += 1
    return n
...
def main():
    ...
    parser.add_argument("--filter-classes", nargs="+", type=int, default=None,
                        metavar="CLASS_ID",
                        help="Keep only these class IDs in label files (writes copies, not symlinks)")
    ...
    filter_classes = set(args.filter_classes) if args.filter_classes is not None else None
    if filter_classes is not None:
        print(f"Filter classes: {sorted(filter_classes)}")
    ...
    for split_yaml in split_files:
        ...
        if args.datasets and dataset not in args.datasets:
            print(f"  skip {dataset} (not in --datasets filter)")
            continue
    ...
    for partition, slices in all_slices.items():
        ...
        counts[partition] = make_symlinks(slices, images_out, labels_out, n_copies_fn, filter_classes)
